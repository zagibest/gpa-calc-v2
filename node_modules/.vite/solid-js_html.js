...
var marker = "<!--#-->";
var reservedNameSpaces = new Set(["class", "on", "oncapture", "style", "use", "prop", "attr"]);
...
function createHTML(r, {
  ...
  function parseKeyValue(tag, name, isSVG, isCE, options) {
    ...
    } else if (namespace !== "attr" && (isChildProp || !isSVG && isProp || isCE || namespace === "prop")) {
...
${childOptions.exprs.join(";\n")};
  ...
  function processComponent(node, options) {
    const keys = Object.keys(node.attrs), props = [], componentIdentifier = options.counter++;
    ...
    if (options.parent)
      options.exprs.push(`r.insert(${options.parent}, r.createComponent(exprs[${componentIdentifier}], r.wrapProps({${props.join(", ") || ""}}))${tag ? `, ${tag}` : ""})`);
    else
      options.exprs.push(`${options.fragment ? "" : "return "}r.createComponent(exprs[${componentIdentifier}], r.wrapProps({${props.join(", ") || ""}}))`);
    options.path = tag;
    options.first = false;
  }
...
var html_default = index;

// dep:solid-js_html
var solid_js_html_default = html_default;
export {
