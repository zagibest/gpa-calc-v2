...
function setAttr$1(path, elem, name, value, { isSVG, dynamic, prevId, isCE }) {
  ...
  if (namespace !== "attr" && (isChildProp || (!isSVG && isProp) || isCE || namespace === "prop")) {
    ...
    return t__namespace.assignmentExpression("=", t__namespace.memberExpression(elem, t__namespace.identifier(name)), value);
  }
          ...
          if (key === "textContent") {
            ...
            results.exprs.push(
              t__namespace.expressionStatement(
                t__namespace.assignmentExpression(
                  "=",
                  t__namespace.memberExpression(elem, t__namespace.identifier("textContent")),
                  value.expression
                )
              ),
              t__namespace.variableDeclaration("const", [
                t__namespace.variableDeclarator(textId, t__namespace.memberExpression(elem, t__namespace.identifier("firstChild")))
              ])
            );
            nextElem = textId;
          }
...
function transformComponentChildren(children) {
  ...
  let transformedChildren = filteredChildren.map(path => {
    if (t__namespace.isJSXText(path.node)) {
      return t__namespace.stringLiteral(trimWhitespace(path.node.extra.raw));
    } else {
  ...
  if (filteredChildren.length === 1) {
    ...
    if (
      ...
      transformedChildren =
        t__namespace.isCallExpression(transformedChildren) && !transformedChildren.arguments.length
